...
public class Main2 {
	...
	public static void main(String[] args) throws Exception{
		...
		for (int i = 0; i < M; i++) {
			for (int k = 0; k < N; k++) {
				for (int f = 0; f < N; f++) {
					System.out.printf("%3d", map[k][f].num);
				}
				System.out.println();
			}
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				if(map[i][j].num > 0) {
					...
					int sp = (cnt == 0 ? 0 : map[i][j].num / cnt);
					
					for (int d = 0; d < 4; d++) {
						int ny = i + dy[d];
						int nx = j + dx[d];
						if(ny < 0 || nx < 0 || ny >= N || nx >= N) continue;
						if(map[ny][nx].num == EMPTY)  temp[ny][nx] += sp;
					}
				}
			}
		}
	...
	public static void findAttack() {
				...
				if(map[i][j].num > 0) {
					...
					for (int d = 0; d < 4; d++) {
						...
						for (int k = 1; k <= K; k++) {
							...
							if(map[ny][nx].num > 0) sumTmp += map[ny][nx].num;
		...
		System.out.println("cnt : " + best.cnt + " y :" + best.y + ", x :" + best.x);
		sum += best.cnt;
	}
	public static void attack() {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				if(map[i][j].num == DEAD) {
					map[i][j].c -= 1;
					if(map[i][j].c == 0) map[i][j].num = EMPTY;
				}
			}
		}
